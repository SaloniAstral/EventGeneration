...
"""
SNS Event Publisher
Sends events when stock data is loaded or updated
 Application events → AWS SNS → Other services
"""
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                # If listing topics fails, try a simple client test
                logger.warning(f"⚠️ SNS topic listing failed: {list_error}")
                logger.info("🔄 Trying simple SNS client test...")
                
                # Test if we can at least create the client and it has basic functionality
                if hasattr(self.sns_client, 'list_topics'):
                    logger.info("✅ SNS client initialized successfully")
                    logger.info(f"Topic ARN: {self.topic_arn}")
                    return True
                else:
                    logger.error("❌ SNS client not properly initialized")
                    return False
